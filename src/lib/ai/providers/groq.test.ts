...
describe("GroqProvider (stub fetch)", () => {
  beforeEach(() => {
  ...
  it("sukses: teks ter-trim + tokensUsed dari usage.total_tokens", async () => {
    ...
    const res = await new GroqProvider().generate(ARGS);
  ...
  it("choices kosong → PROVIDER_DOWN", async () => {
    ...
    const err = await aiErrorOf(new GroqProvider().generate(ARGS));
...
describe.skipIf(!process.env.GROQ_API_KEY)("GroqProvider (live)", () => {
